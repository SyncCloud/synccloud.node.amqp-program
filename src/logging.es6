...
// export results in weird bug - don't use it for the moment
...
class Logger {
    ...
    post(level, messageFn, reprFn) {
        try {
            const event = {
                ...
                level: level, message: messageFn(),
                timestamp: moment.utc()
            };
            try {
                // console.log('stringify(event)', event);
                ...
                const repr = reprFn ? () => {
                    if (!repred) {
                        reprCache = reprFn(event);
                        repred = true;
                    }
                    return reprCache;
                } : null;

                this._post(json, event, repr);
            }
            catch (exc2) {
                console.warn('Failed to log an event:');
                console.error(exc2.stack || exc2);
                console.error(event);
            }
        }
        catch (exc) {
    ...
    _post(json, event, reprFn) {
        this.targets.forEach((target) => {
            target.post(this, json, event, reprFn);
        });
    }
}
        ...
        this._formatters = [
            {
                ...
                format: (obj) => obj.format('HH:mm:ss.SSSSSSSSS')
            }
        ];
    }
